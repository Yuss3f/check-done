document.addEventListener("DOMContentLoaded", function () {
  ...
  async function deleteTaskFromServer(taskText) {
    try {
      ...
      if (taskToDelete) {
        const deleteResponse = await fetch(`http://localhost:5500/tasks/${taskToDelete._id}`, {
          method: "DELETE",
          credentials: "include"
        });
  ...
  async function updateTaskStatus(taskText, isCompleted) {
    try {
      ...
      if (taskToUpdate) {
        const updateResponse = await fetch(`http://localhost:5500/tasks/${taskToUpdate._id}`, {
          method: "PUT",
          headers: {
            "Content-Type": "application/json"
          },
